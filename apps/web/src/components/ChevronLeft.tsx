...
interface ChevroneLeftProps {
  scrollToLeft: () => void;
  timerActive: () => void;
  isScrolling: boolean;
}

const ChevroneLeft: React.FC<ChevroneLeftProps> = ({
  scrollToLeft,
  timerActive,
  isScrolling,
}) => {
  return (
    <button
      className={`absolute left-1 transform top-[35%] text-white z-10`}
      onClick={() => {
        scrollToLeft();
        timerActive();
      }}
    ...
    >
      <div className="group bg-black bg-opacity-35 border-white border-[1px] rounded-lg px-3 py-5">
        <svg
          ...
          className="group-hover:text-[#F5C518] transition-colors duration-300 w-[24px]"
          viewBox="0 0 24 24"
          fill="currentColor"
          role="presentation"
        >
