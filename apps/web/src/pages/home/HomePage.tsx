...
import { AffiliateLinks, GenreSpotlightCarousel, SectionDivider, Spinner } from "../../components";
...
export function HomePage() {
  const [topRatedMovies, setTopRatedMovies] = useState<MediaListItem[]>([]);
  const [upcomingMovies, setUpcomingMovies] = useState<MediaListItem[]>([]);
  const [topRatedTvShows, setTopRatedTvShows] = useState<MediaListItem[]>([]);
  const [onTheAirTvShows, setOnTheAirTvShows] = useState<MediaListItem[]>([]);
  const [heroCarousel, setHeroCarousel] = useState<MediaListItem[]>([]);
  const [actors, setActors] = useState<MediaListItem[]>([]);
  const [trendingMovies, setTrendingMovies] = useState<MediaListItem[]>([]);
  const [thisYearHighlights, setThisYearHighlights] = useState<MediaListItem[]>([]);
  const [trendingTv, setTrendingTv] = useState<MediaListItem[]>([]);
    ...
    const loadHomeData = async () => {
      try {
        ...
        if (
          ...
          setThisYearHighlights(yearHighlightsData?.results ?? []);
