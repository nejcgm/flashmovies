...
const ActorCarousel = ({ actors, showTitle, cardCount, scrollDistance }) => {
  ...
  return (
    <>
      <div className="max-w-[1250px] relative">
        <div className="text-[#F5C518] font-roboto font-bold text-[32px] first-letter:uppercase mb-[16px]">
          {showTitle}
        </div>
        ...
        <div
          ...
          ref={scrollContainer}
        >
          {actors?.slice(0, cardCount).map((item, index) => (
            <ActorCard
              key={index}
              name={item.name}
