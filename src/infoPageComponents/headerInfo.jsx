    ...
    <>
      ...
      <div className='font-roboto text-white'>
        <div className='flex w-full justify-between items-center'>
          <div>
          <div className='capitalise text-[32px] xl:text-[48px] flex items-center gap-3'>
          <BackButton/>
            <div>{title}</div> 
            <button onClick={()=>{navigator.clipboard.writeText(urlToCopy)}}>
        ...
        <div className='flex w-full mt-[24px] gap-2'>
          ...
          onClick={()=>{navigate(`/fullmovie/?id=${movieId}&type=${type}`)}}
          >
      ...
      <div className='flex lg:hidden'>
        ...
        <div className='mt-[8px] lg:mt-[16px]'>{(description)?.slice(0,expand)}
            <button onClick={()=>{setExpand((prevValue) => (prevValue === 500 ? ContentLength : 500))}} 
            className='text-[#BBBBBB] text-[14px]'
            >
            {ContentLength < 500 ? (' ') :  expand == 500 ? (' ...Read More') : (' ...View Less')}
            </button></div>
        </div>
        </div>
      </div>
