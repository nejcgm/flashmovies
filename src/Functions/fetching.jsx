...
//info search = ''
//videons search = videos
//related search = simiral
//reviews search = reviews
//actors type = person movieid = movieid  search=popular page=page

export const fetchSpecific = async (type,movieId,search,genreList,page) => {
    
    const url = `https://api.themoviedb.org/3/${type}/${movieId}${search}?language=en-U${genreList && `&with_genres=${genreList.join(",")}`}${page && `&page=${page}&sort_by=popularity.desc`}`;
    const options = {
        method: 'GET',
        headers: {
