...
import ListItem from '../listMovie/listItem';
import { fetchSearch,useDebounce } from '../Functions/fetching';
    ...
    <>
        ...
        <div className=' w-full items-center justify-center flex mt-[32px] mb-[32px] gap-4'>
            ...
            <div className=' w-[40%] flex-col'>
                ...
                <input
                    className='relative z-20 font-roboto py-2 px-6 rounded-lg w-full focus-none outline-offset-[-2px] outline-[#F5C518] '
                    type="text"
                ...
                {query &&
                    <div className='relative w-full z-20'>
                        <div className='absolute p-6 bg-[#1F1F1F] w-full flex flex-col gap-4 rounded-lg'>
                            {MovieSearch.slice(0, cardCount).map((item, index) => (
                                <ListItem
                                    key={index}
                                    ...
                                    onCancel={(e) => { setQuery(null) }}
                                    largeScreen={false}
                                />
                            ))}
                            <button onClick={() => { setCardCount((prevValue) => (prevValue === 5 ? 10 : 5)) }}
                                className='text-white text-[14px] font-roboto'
                            >
                                {cardCount == 5 ? ('View More') : ('view Less')}</button>
                        </div>
                    </div>
                }
